...
CREATE TABLE "MEMBER" (
	...
	"MEM_QUIT_DATE"	DATE	DEFAULT SYSDATE	NULL,
...
COMMENT ON COLUMN "QUIT"."MEM_EMAIL" IS '이메일';

COMMENT ON COLUMN "QUIT"."MEM_NICK" IS '회원 닉네임(한글 2~8자)';

COMMENT ON COLUMN "QUIT"."MEM_PASSWORD" IS '비밀번호 (영문 대문자, 소문자, 숫자 포함 8~20자)';

COMMENT ON COLUMN "QUIT"."MEM_ROLE" IS '회원 권한 등급 [휴면회원(ROLE_SLEEP), 일반회원(ROLE_MEM), 폰트적용회원(ROLE_VIP), 관리자(ROLE_ADMIN), 소유자(ROLE_OWNER)]';

COMMENT ON COLUMN "QUIT"."MEM_ENABLED" IS '활성 여부 [비활성(0), 활성(1)]';

COMMENT ON COLUMN "QUIT"."MEM_TYPE" IS 'SNS 여부[이메일(T), 카카오(K), 네이버(N), 구글(G)]';

COMMENT ON COLUMN "QUIT"."MEM_JOIN_DATE" IS '가입날짜';

COMMENT ON COLUMN "QUIT"."MEM_QUIT_DATE" IS '탈퇴날짜';

COMMENT ON COLUMN "QUIT"."MEM_BIRTH" IS '생일';
...
CREATE TABLE "PLACE" (
	...
	"MLEVEL"	CHAR(2)		NULL,
	"SIGUNGUCODE"	VARCHAR2(2)		NOT NULL,
...
COMMENT ON COLUMN "PLACE_MOVE_TIME"."TYPE" IS '장소 타입(필요한것만 분류, 나머지 0)';

COMMENT ON COLUMN "PLACE_MOVE_TIME"."CONTENTID_START" IS '장소 ID';

COMMENT ON COLUMN "PLACE_MOVE_TIME"."CONTENTID_END" IS '장소 ID';
