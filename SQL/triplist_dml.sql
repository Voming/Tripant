...
--공유자
select mem_nick, plan_id, plan_mem_role from plan_member join member using (mem_email) where plan_id = 13 and plan_mem_role = 0
union
select mem_nick,  null, null from member where mem_role in ('ROLE_MEM', 'ROLE_VIP') and mem_nick like '%원%'
minus
select mem_nick,  null, null from member join plan_member using (mem_email) where plan_id = 13
;
----공유자 더미
